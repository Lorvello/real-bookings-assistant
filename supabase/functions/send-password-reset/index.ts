...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Extract tokens from Supabase URL and build custom reset URL
    const supabaseActionLink = data.properties?.action_link;
    if (!supabaseActionLink) {
      console.error("❌ No action link generated by Supabase - full response:", JSON.stringify(data, null, 2));
    ...
    // Parse the Supabase URL to extract tokens
    const actionUrl = new URL(supabaseActionLink);
    const token = actionUrl.searchParams.get('token');
    const type = actionUrl.searchParams.get('type');
    
    if (!token || type !== 'recovery') {
      console.error("❌ Invalid tokens in action link:", { token: token ? '***' : null, type });
      return new Response(JSON.stringify({ error: 'Failed to generate valid reset tokens' }), {
        status: 500,
        headers: { "Content-Type": "application/json", ...corsHeaders },
      });
    }

    // Build custom reset URL with extracted tokens - direct to reset page
    const resetUrl = `${baseUrl}/reset-password?token=${token}&type=${type}`;
    
    console.log("🔗 Extracted tokens from Supabase URL");
    console.log("🔗 Built custom reset URL:", resetUrl);
