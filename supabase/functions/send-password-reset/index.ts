...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const originalResetUrl = data.properties?.action_link;
    if (!originalResetUrl) {
    ...
    // Parse the original Supabase URL to extract tokens
    const originalUrl = new URL(originalResetUrl);
    console.log("🔗 Original Supabase URL:", originalResetUrl);
    
    // Create our custom reset URL with the tokens but pointing to our domain
    const customResetUrl = new URL(`${baseUrl}/reset-password`);
    // Copy all query parameters (tokens) from Supabase URL
    originalUrl.searchParams.forEach((value, key) => {
      customResetUrl.searchParams.set(key, value);
    });
    
    const resetUrl = customResetUrl.toString();
    console.log("🔗 Custom reset URL:", resetUrl);
    console.log("🔗 Reset URL domain:", customResetUrl.origin);
    console.log("🔗 Reset URL path:", customResetUrl.pathname);
