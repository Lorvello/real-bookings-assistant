...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Microsoft OAuth token exchange:', { code: code?.substring(0, 10) + '...', state, user_id })
    ...
    const clientId = Deno.env.get('VITE_OUTLOOK_CLIENT_ID')
    ...
    const redirectUri = `${Deno.env.get('SUPABASE_URL')}/auth/outlook/callback` || 'http://localhost:5173/auth/outlook/callback'
    ...
    console.log('User info:', { email: userInfo.mail || userInfo.userPrincipalName, id: userInfo.id })
