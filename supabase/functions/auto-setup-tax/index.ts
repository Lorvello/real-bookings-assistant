...
serve(async (req) => {
  ...
  try {
    ...
    // now INERT for key/env selection. The sub-invoked tax fns (detect-tax-requirements,
    // manage-tax-registrations) also derive mode server-side, so the test_mode forwarded
    // in their bodies is likewise inert. Defaults to test when STRIPE_MODE is unset.
