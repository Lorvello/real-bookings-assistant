-- Project enabled_payment_methods + payment_deadline_hours + auto_cancel_unpaid_bookings
-- into business_overview (V1). Goal priority #3 explicitly asks for these. The
-- payment_settings table is already LEFT JOINed in refresh_business_overview and its
-- AFTER-ROW trigger refreshes on any change, so no trigger change is needed. Verified
-- live: a payment_settings update lands in business_overview, then reverts clean.
-- (Whether the n8n agent's tool returns these to the LLM is Luciano's node config;
-- the data is now present in the projection.)

ALTER TABLE public.business_overview
  ADD COLUMN IF NOT EXISTS enabled_payment_methods jsonb,
  ADD COLUMN IF NOT EXISTS payment_deadline_hours numeric,
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO business_overview (
      ...
      website, instagram, facebook, linkedin, tiktok, youtube, x,
      ...
      enabled_payment_methods, payment_deadline_hours, auto_cancel_unpaid_bookings,
    ...
    SELECT
      ...
      u.website, u.instagram, u.facebook, u.linkedin, u.tiktok, u.youtube, u.x,
      ...
      ps.enabled_payment_methods, ps.payment_deadline_hours, ps.auto_cancel_unpaid_bookings,
      '[]'::jsonb,
      COALESCE((
        SELECT jsonb_agg(
          jsonb_build_object(
