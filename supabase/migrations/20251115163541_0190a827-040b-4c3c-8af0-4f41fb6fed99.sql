-- Step 1: Fix the refresh_business_overview function
CREATE OR REPLACE FUNCTION public.refresh_business_overview(p_calendar_id UUID DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- Step 2: Fix RLS policies on business_overview
DROP POLICY IF EXISTS business_overview_owner_all ON business_overview;
DROP POLICY IF EXISTS business_overview_public_read ON business_overview;
DROP POLICY IF EXISTS business_overview_owner_read ON business_overview;
DROP POLICY IF EXISTS business_overview_owner_insert ON business_overview;
DROP POLICY IF EXISTS business_overview_owner_update ON business_overview;
DROP POLICY IF EXISTS business_overview_owner_delete ON business_overview;

-- Owner insert (restrictive)
CREATE POLICY business_overview_owner_insert
...
-- Owner update (restrictive)
CREATE POLICY business_overview_owner_update
ON business_overview
AS RESTRICTIVE
FOR UPDATE
TO authenticated
...
-- Owner delete (restrictive)
CREATE POLICY business_overview_owner_delete
ON business_overview
AS RESTRICTIVE
FOR DELETE
TO authenticated
...
-- Public read for active calendars (permissive)
...
-- Owner can always read their own calendars (permissive)
CREATE POLICY business_overview_owner_read
ON business_overview
AS PERMISSIVE
FOR SELECT
TO authenticated
