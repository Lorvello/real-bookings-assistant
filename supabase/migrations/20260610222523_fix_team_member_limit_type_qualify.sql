-- Fix: check_team_member_limit failed with 'type subscription_tier does not exist'
-- because the ::subscription_tier cast was unqualified while the function runs with
-- search_path=''. This broke team invitations (invite_team_member calls it).
...
-- Fix: invite_team_member used encode(...,base64url) which Postgres does not support -> URL-safe base64.
