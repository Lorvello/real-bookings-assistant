-- Fix user status logic for reactivation and add missed payment status
...
BEGIN
  ...
  -- 2) Check for missed payment status first (past_due or incomplete)
  IF v_user.subscription_status IN ('past_due', 'incomplete') THEN
