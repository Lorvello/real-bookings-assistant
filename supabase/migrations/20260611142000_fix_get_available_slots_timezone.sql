-- Fix timezone handling in get_available_slots.
--
-- The window bounds were built as:
--   (p_date + rule.start_time)::timestamp with time zone AT TIME ZONE p_timezone
-- The `::timestamp with time zone` cast first reinterprets the local wall-clock
-- time in the SESSION timezone (UTC), then `AT TIME ZONE` converts it again —
-- producing slots offset by the timezone amount (e.g. 09:00 Europe/Amsterdam
-- surfaced as 11:00 UTC instead of 07:00 UTC). Customers were shown availability
-- at the wrong times entirely.
--
-- Correct form: `(p_date + rule.start_time) AT TIME ZONE p_timezone` — applying
-- AT TIME ZONE to a timestamp-WITHOUT-time-zone interprets the wall-clock as
-- being in p_timezone and yields the right timestamptz.
