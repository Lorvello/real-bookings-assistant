-- Admin function to consistently apply developer-selected user status across users, subscribers, and calendars
-- This centralizes all logic to prevent mismatches (e.g., subscribers still marked subscribed while users set to expired)
...
BEGIN
  ...
  ELSIF p_status = 'active_trial' THEN
    ...
    PERFORM 1 FROM public.subscribers WHERE user_id = p_user_id INTO v_existing_sub;
    IF NOT FOUND THEN
      INSERT INTO public.subscribers (user_id, email, subscribed, subscription_end, subscription_tier)
      VALUES (p_user_id, COALESCE((SELECT email FROM public.users WHERE id = p_user_id), ''), false, NULL, NULL);
    ELSE
