...
export function PayAndBookHeader({
  ...
  return (
    <SettingsSection
      icon={Shield}
      title="Pay & Book"
      description="Collect secure pre-payments on bookings to cut no-shows and lock in revenue upfront — paid out to your own account through Stripe."
      action={
        <Switch
          checked={enabled}
          onCheckedChange={onToggle}
          disabled={disabled || !isSetupComplete}
          aria-label="Enable Pay & Book"
          data-pay-book-toggle
        />
      }
    >
      <p className="text-sm leading-6 text-muted-foreground">
        When enabled, customers can pay for a booking online — through WhatsApp or
        your booking page — before the slot is confirmed.
      </p>
