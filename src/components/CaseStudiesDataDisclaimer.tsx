  ...
  return (
    <div className="-mt-4 pb-8 md:pb-12 text-center">
      <p className="text-slate-400 text-[8px] sm:text-sm mb-1 sm:mb-2 font-medium">
        ...
        {t('whyus.caseDisclaimer.text', 'Based on verified business performance data from in-depth case studies')}
      </p>
      ...
      <div className="flex flex-row items-center justify-center gap-1 sm:gap-6 text-[7px] sm:text-xs text-slate-400">
        ...
        <span>{t('whyus.caseDisclaimer.tag1', '6-12 month performance tracking')}</span>
        <span className="inline sm:inline text-slate-600">•</span>
        <span>{t('whyus.caseDisclaimer.tag2', 'Results vary by implementation')}</span>
      </div>
    </div>
  );
};
