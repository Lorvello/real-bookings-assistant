...
const PainPoint = ({ icon: Icon, title, description, color }: PainPointProps) => {
  return (
    <div className="group text-center hover:transform hover:scale-105 transition-all duration-300 cursor-pointer bg-slate-800/30 md:bg-transparent rounded-2xl p-4 md:p-0 h-full flex flex-col">
      {/* Clean, minimal icon */}
      ...
      <div className="flex-1 flex flex-col">
        <h3 className="text-base md:text-xl sm:text-2xl font-bold text-white mb-3 md:mb-3 sm:mb-4 leading-tight group-hover:text-red-300 transition-colors duration-300">
          {title}
        </h3>
        
        {/* Simple description - Better mobile spacing */}
        <p className="text-slate-300 text-sm md:text-base sm:text-lg leading-relaxed max-w-sm mx-auto group-hover:text-slate-200 transition-colors duration-300 flex-1">
          {description}
        </p>
      </div>
    </div>
