...
/**
 * Auth is provided ONCE at the app root via <AuthProvider> and read through
 * useAuth(). Previously useAuth was a plain hook imported by ~45 components, so
 * each mounted consumer spun up its own onAuthStateChange subscription, its own
 * session-timeout window listeners, and logged every auth event — flooding the
 * console with duplicate "Auth event: INITIAL_SESSION" lines and wasting work.
 * Centralising it means a single subscription and a single source of truth.
 */
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      (event, session) => {
        ...
        // Log meaningful auth transitions for security monitoring. INITIAL_SESSION
        // fires on every load/tab and isn't a security event — skip it to keep
        // the console clean.
